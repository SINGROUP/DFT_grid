...
from plot_efield import plot_Ez_xz, plot_Ez_yz
...
file_path = 'LOCPOT'
plot_filename = 'locpot_efield.png'
tip_atom_type = 8
grid_spacing = 0.1
plot_pos = [0, 0, 4.0]
plot_box_size = 12.0
atom_radius_scaling = 0.3
efield_xz_limits = [-1.0, 1.0]
efield_yz_limits = [-1.0, 1.0]
...
plot_Ez_xz(xs, ys, zs, pot_on_grid, plot_pos[1], limits=efield_xz_limits)
...
plt.title(u'Electric field (xz cut through tip)', size=16)
plt.xlabel(u'x (Å)', size=14)
plt.ylabel(u'z (Å)', size=14)

ax2 = plt.subplot(2, 1, 2)
plot_Ez_yz(xs, ys, zs, pot_on_grid, plot_pos[0], limits=efield_yz_limits)
plot_atoms(atom_types, atom_positions, normal_direction='x', atom_radius_scaling=atom_radius_scaling)
plt.xlim(-plot_box_size/2, plot_box_size/2)
plt.ylim(-plot_box_size/2, plot_box_size/2)
ax2.set_aspect('equal')
plt.title(u'Electric field (yz cut through tip)', size=16)
